...
- If the description includes a `## Intent` section with specific claims (problem, scope, non-goals, key decisions), check whether the diff delivers each claim. A claim with no supporting code change, or a code change that contradicts a stated non-goal, is a finding — report it under category `Intent Mismatch`. If no `## Intent` section is present, skip this check; never invent intent.
```
