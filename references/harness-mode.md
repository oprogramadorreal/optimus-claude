...
- **code-review**: whatever the working tree holds, review in Step 3's **Branch/ref mode** with `<ref>` = `config.scope.base_ref`, else `config.pr_description.base_ref`, else `origin/<default branch>` per Step 3 item 3 (never ask the user; if none resolves, set `<ref>` to the empty tree from `git hash-object -t tree /dev/null` and diff it two-dot against `HEAD`, so the whole path scope is reviewed — an empty block for an unreviewed scope would end the run as a clean `convergence`), filtered to `config.scope.paths` when non-empty — never PR mode, never `gh pr view`; skip the large-diff warning.
