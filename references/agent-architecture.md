...
Both agents read the target project's `.claude/docs/coding-guidelines.md` and `.claude/CLAUDE.md` for project-specific standards. In projects with a skill-authoring stack, they also read `.claude/docs/skill-writing-guidelines.md` and route each analyzed file to the correct lens (coding-guidelines for code files, skill-writing-guidelines for markdown instruction files) — the code-simplifier inlines equivalent routing rules directly to stay within the reference-depth limit.
