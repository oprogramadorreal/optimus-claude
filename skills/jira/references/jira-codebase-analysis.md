...
Markdown template for the analysis comment (Rovo and sooperset both take Markdown, not wiki markup). Derive each section from the corresponding section just written to the local file; write the comment in the JIRA issue's original language.

```
## Codebase Analysis (automated)
