...
2. **Intent check** — look for comments, test assertions, or established patterns that explain the code's behavior (what looks like a bug may be intentional). **Recent change-intent awareness:** for each unique file with findings, run `git log --no-merges --format="%h %s" -5 -- <file>` to retrieve the last 5 non-merge commits touching that file. If any of those commits clearly indicates the flagged code was deliberately introduced (e.g., commit message references a fix, feature, or security hardening), and the finding suggests removing, reverting, or simplifying that deliberate code, reduce confidence by one level. If a commit message is uninformative (under ~15 characters, or generic like "fix", "update", "wip"), run `git show <sha> -- <file>` to check whether that commit introduced the lines being flagged — look for patterns like null checks, validation, error handling, or security measures that reveal intent. If intent remains unclear after checking the diff, this check has no effect. Applied uniformly across all finding categories. This is a soft adjustment — never a hard filter. If `git log` fails or returns no results (shallow clone, new file), skip this check for that file.
