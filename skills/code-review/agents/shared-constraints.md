...
**Field names for Intent Mismatch findings.** All agents emit `Current:`/`Suggested:` for Intent Mismatch findings to match the aggregator's expected shape (see `skills/code-review/SKILL.md` "Output format"). Agents whose default output format uses `Code:`/`Fix:` (bug-detector, security-reviewer) must rename those fields to `Current:`/`Suggested:` for Intent Mismatch findings only.
