...
- **Harness mode (iterations 2+)**: When running under `HARNESS_MODE_INLINE` and the progress file's `iteration.current` > 1, inject the **Iteration Context Block** before the file list line in every agent prompt.
- **Both apply**: If both PR/MR context and iteration context apply (harness mode on a PR), inject PR/MR context first, then iteration context, both before the file list line.
