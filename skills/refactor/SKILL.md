...
4 analysis agents launched in parallel for maximum coverage.
...
Read the agent prompt files from `$CLAUDE_PLUGIN_ROOT/skills/refactor/references/agents/` for individual agent prompts. Read `$CLAUDE_PLUGIN_ROOT/skills/refactor/references/agents/shared-constraints.md` for the shared quality bar, exclusion rules, and false positive guidance that apply to all agents.

### Iteration context injection (deep mode, iterations 2+)

If deep mode is active and `iteration-count` > 1, prepend the iteration context block to every agent prompt before the file list. Read `$CLAUDE_PLUGIN_ROOT/skills/refactor/references/agents/context-blocks.md` for the template and format.
