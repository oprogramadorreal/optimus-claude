...
> **Coming from plan mode?** TDD runs in **normal mode**, in a fresh conversation. If you were iterating on a plan-mode prompt generated by `/optimus:brainstorm` or `/optimus:jira`, exit plan mode (without approving it) and start a fresh conversation before invoking this skill — TDD will auto-detect the updated design or JIRA doc.
