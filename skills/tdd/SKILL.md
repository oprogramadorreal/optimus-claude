...
4. **Spec auto-discovery** — if no design doc or JIRA context resolved (none found, or the user ignored them) but `docs/specs/` exists with `.md` files, select the most recent (by filename date prefix if present, otherwise file modification time). If it is within the last 7 days, mention it: "Found build spec `<path>` — use it as the basis for TDD?" via `AskUserQuestion` — header "Spec", options "Use it" / "Ignore — describe a different task". If older than 7 days, add a note: "(This spec is [N] days old.)" A `docs/specs/<spec>.md` is a human-authored build spec — the bring-your-own-spec slot in `$CLAUDE_PLUGIN_ROOT/references/sdd-mapping.md`; use its Goal and Acceptance Criteria as the task description (optimus authors nothing there). If it has a `## Scenarios` section in Given/When/Then form, the Step 3 scenario-driven shortcut applies.

5. **No context found** — proceed with normal task gathering below.

Precedence (first match wins): design docs → JIRA context → `docs/specs/` build specs → none. Design docs take priority because they are the most detailed (they incorporate JIRA context if brainstorm consumed it); a human-authored `docs/specs/` spec is the final fallback when no optimus-native context exists. Detected context feeds into the existing task-gathering cascade below — it does NOT bypass Step 3 decomposition. TDD still independently decomposes the goal into behaviors, except when a `## Scenarios` section is present (see the scenario-driven shortcut in Step 3).
