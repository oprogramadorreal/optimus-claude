...
**Match precedence.** A detected tool may match multiple rows (e.g., the same DBeaver install matches a project with both Postgres and MongoDB services). Render one H4 per (tool, External Services row) hit — there is no "primary service" reduction. The maximum H4 count for a project is `|Recommended Developer Tools ∩ Catalog| × |External Services rows ∩ Matched service tokens|`, capped at 20 per External Services per-service heading (rare; if exceeded, render the first 20 in detector-reported × External-Services-reported order and emit a single overflow line `+N more — see <tool> docs`).
...
3. **Render the H4** under the matching External Services per-service heading, after the existing snippet, source citation, env-var notes, and the Pre-Conditions block (or its `- Connection details for <…>` bullet replacement):
