...
Cross-product of database / cache GUI clients and the External Services they connect to. Step 4 of [`SKILL.md`](../SKILL.md) emits a `#### Connecting from <tool>` H4 inside an External Services service's H3 when both of the following hold:
...
**Match precedence.** A detected tool may match multiple rows (e.g., the same DBeaver install matches a project with both Postgres and MongoDB services). Render one H4 per (tool, External Services row) hit — there is no "primary service" reduction. The maximum H4 count for a project is `|Recommended Developer Tools ∩ Catalog| × |External Services rows ∩ Matched service tokens|`, capped at 20 per project H3 (rare; if exceeded, render the first 20 in detector-reported × External-Services-reported order and emit a single overflow line `+N more — see <tool> docs`).
...
1. **Resolve the matching External Services H3.** Whole-token match (case-insensitive) the catalog row's *Matched service tokens* against the External Services row's `Service` field. When a tool maps to multiple services (DBeaver / DataGrip multi-service rows), emit one H4 per matching service.
2. **Substitute placeholders from the rendered snippet** of that H3:
...
3. **Render the H4** under the matching External Services H3, after the existing snippet, source citation, env-var notes, and Pre-conditions / Connection details bullets:

   ````markdown
   #### Connecting from <tool>
...
4. **Cite the canonical source** using the `- Source: [<title>](<url>)` format from [`external-services-docker.md`](external-services-docker.md) §Citation Format. Apply that section's host-extraction allowlist to the citation URL — when the row's URL fails the allowlist (e.g., a typo in the seed), drop the H4 entirely rather than rendering with no citation. Citation hosts permitted for GUI-tool docs (in addition to those already permitted by §Citation Format): the tool vendor's docs subdomain matched against the GUI-client name (`pgadmin.org`, `dbeaver.com`, `studio3t.com`, `redis.io`, `dev.mysql.com`, `jetbrains.com`, `mongodb.com`, `learn.microsoft.com`).
5. **Multiple-tool case.** When two tools in the detector's *Recommended Developer Tools* match the same External Services row (e.g., both `pgAdmin` and `DBeaver` are listed for a Postgres service), render one H4 per matching tool in the detector-reported order. Each H4 cites its own row's `Canonical source`.

## Web-Search Recipe re-validation

Before emitting any `#### Connecting from <tool>` H4, run [`external-services-docker.md`](external-services-docker.md) §Web-Search Recipe step 3 (`WebFetch`) against the row's `Canonical source` URL with the prompt:
