...
2. **Substitute placeholders from the rendered snippet** of that per-service heading:
   - `<host>` → per [`external-services-docker.md`](external-services-docker.md) §Pre-Conditions Block §Substitution rules `<host>` rule (Windows host → `127.0.0.1`; else `localhost`).
...
3. **Render the heading** one level deeper than the External Services per-service heading (H4 in multi-repo, H5 in single-project / monorepo), placed after the existing snippet, source citation, env-var notes, and either the Pre-Conditions block (when one fires) or the snippet's `- Connection details for <…>` bullet (when no Pre-Conditions block fires) — exactly one of the two is present per [`external-services-docker.md`](external-services-docker.md) §Pre-Conditions Block §Step 6 audit. Substitute `<heading-prefix>` with `####` in multi-repo and `#####` in single-project / monorepo BEFORE writing the file — the literal `<heading-prefix>` token must never appear in the rendered output:
...
4. **Cite the canonical source** using the `- Source: [<title>](<url>)` format from [`external-services-docker.md`](external-services-docker.md) §Citation Format. The `- Source: [` prefix and the URL-shape rules (https-only, no `@` / `\` / unencoded whitespace / control chars / non-ASCII in the host) still apply. For `Connecting from <tool>` citations, the GUI-client docs allowlist replaces §Citation Format's host-equality table and `/docs/`-path requirement — apply only the URL-shape rules above plus host membership in the allowlist (host-equality, lowercased): `pgadmin.org`, `dbeaver.com`, `studio3t.com`, `redis.io`, `dev.mysql.com`, `jetbrains.com`, `mongodb.com`, `learn.microsoft.com`. When the row's URL fails the allowlist (e.g., a typo in the seed), drop the heading entirely rather than rendering with no citation.
