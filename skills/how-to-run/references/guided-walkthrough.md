...
- `\b<DOWNLOADER>\b.*\|\s*((sudo|doas|pkexec|run0|su\s+-c|gosu|setpriv|nsenter|unshare|env|runuser|machinectl\s+shell|stdbuf(\s+-\S+)*|time|timeout(\s+\S+)?|nice|ionice|nohup|exec|chroot|xargs(\s+-\S+)*(\s+-[IJ]\s+\S+)?|parallel(\s+-\S+)*)[^|]*?\s+)?<EXECUTOR>\b`  (covers `curl|bash`, `wget|bash`, and the cross-shell pipe-to-executor variants enabled by the full `<DOWNLOADER>` set above. Wrapper alternation matches four categories: privilege escalation (`sudo`/`doas`/`pkexec`/…), environment prefix (`env`/`runuser`/`machinectl shell`/`stdbuf`), passthrough / scheduler / chroot (`time`/`timeout`/`nice`/`ionice`/`nohup`/`exec`/`chroot`), and stdin-driver tools (`xargs`/`parallel`) — `curl URL | xargs -I {} bash -c '{}'` substitutes the fetched content into a `bash -c` command and is RCE-equivalent to `curl|bash`. `timeout` and `nohup` are common in install docs; `exec` is the canonical "replace shell with downloaded code" form.)
