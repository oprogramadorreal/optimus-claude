...
- `\b<DOWNLOADER>\b.*\|\s*((sudo|doas|pkexec|run0|su\s+-c|gosu|setpriv|nsenter|unshare|env|runuser|machinectl\s+shell|time|nice|ionice|chroot)[^|]*?\s+)?<EXECUTOR>\b`  (covers `curl|bash`, `wget|bash`, and the cross-shell pipe-to-executor variants enabled by the full `<DOWNLOADER>` set above. Wrapper alternation matches the same three categories defined in the curl two-step rule below — privilege escalation, environment prefix, measurement/scheduler/chroot.)
...
- `<EXECUTOR>\s+(-\S+\s+)*-c\s+["']?(\$\(|\x60)\s*\b<DOWNLOADER>\b`  (the Homebrew / rustup / oh-my-zsh / nvm canonical install form: `bash -c "$(curl …)"`, `sh -c \`curl …\``, `zsh -c "$(wget …)"`, `python3 -c "$(curl …)"`, plus the modern equivalents `bash -c "$(http GET …)"`, `pwsh -c "$(iwr …)"`. Most-cited remote-code-executor pattern on the modern developer web.)
