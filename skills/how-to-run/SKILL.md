...
- **Workspace kind** (one of `npm-workspaces` / `pnpm-workspaces` / `yarn-workspaces` / `lerna` / `nx` / `turbo` / `cargo-workspace` / `go-workspace` / `gradle-multi-module` / `maven-multi-module` / `none`). Drives which install/build/run commands Step 4 writes — e.g., `cargo build --workspace` for `cargo-workspace`, `go work sync` for `go-workspace`, `./gradlew build` for `gradle-multi-module`, `npx nx run-many -t build` for `nx`. Never emit workspace-wide commands that don't match the detected kind.
...
- **Runnable components** (per-row of the detector's Components table, e.g., "Isa360.Web (web) — requires SQL Server, Redis; Isa360.WebWorkerManager (worker) — requires SQL Server, Redis, AWS SNS; isa360-ngapp (frontend) — requires Isa360.Web" — or "single component" / "none detected"). Step 4 renders one `#### <Component> (shell N)` subsection per component in Running in Development, with a `Boot order:` header and per-component `Requires:` list.
- **Runtime bind ports** (per-component bound port from launch configs or source grep, e.g., "Isa360.Web: 51914 (launchSettings.json:6), worker: 51915 (launchSettings.json:20)" — or "none detected" when the detector's Runtime Ports table is empty). These are the ports Step 4 may quote in `Expected result:` URLs; any port the detector could not attribute to a file MUST be absent from this bullet.
...
3. **Source Dependencies** — fix-after-clone only: `git submodule update --init --recursive` when `.gitmodules` is present (the primary `git clone [--recursive]` lives in Installation, section 4), sibling repos that must be cloned alongside this one (list paths + clone URLs), CMake `FetchContent`/`ExternalProject` notes.
...
- **Never guess runtime ports.** Runtime port numbers in `Expected result:` lines, troubleshooting bullets, and any URL of the form `http://localhost:<N>` or `http://127.0.0.1:<N>` must come from the Step 1 Checkpoint *Runtime bind ports* bullet (sourced from the detector's Runtime Ports table) OR from the detector's External Services table Port column (compose / config-file ports). When no bound port is found for a component, OMIT the port from the `Expected result:` URL — write `Expected result: <component> starts listening (see <launch-config-file> for the bound port).` Never fall back to a framework default (ASP.NET `5000`, Rails `3000`, Django `8000`, Spring Boot `8080`) because model-memory defaults frequently contradict the project's actual configured port.
...
- **Specific-Token Audit** — prevents hallucinated ports, paths, versions, and counts from reaching the reader. Build a `grounded-tokens` set from the detector's return format:
  ...
  - **Port check:** any integer 1–65535 that appears immediately after `port`, `localhost:`, `127.0.0.1:`, `listens on`, `bound to`, `listening at`, `:`-prefixed at the end of a `http(s)://<host>:` URL, or inside an `Expected result:` line, must appear in the grounded ports set. On miss, re-read the cited Runtime Ports source file; if the file's current value differs from the generated text, reject and force either a rewrite with the observed value or deletion of the port (per the *Never guess runtime ports* Content Principle — omit the URL, do not substitute a framework default).
  - **Path check:** every `<dir>/<subdir>` or `<name>.<ext>` fragment in Installation / Source Dependencies / Environment Setup / Running in Development / Common Issues prose must either (a) be in the grounded paths set verbatim, OR (b) still exist on the filesystem. For fragments that contain `/`, `Glob` the literal fragment. For bare-filename fragments (no `/`, e.g., `appsettings.Development.json`, `docker-compose.yml`), `Glob` `**/<name>.<ext>` instead — a literal-name glob only matches root-level files and would reject every legitimate reference to a deep config file. Reject paths that fail both checks (per the *Never assert an unobserved path* Content Principle).
