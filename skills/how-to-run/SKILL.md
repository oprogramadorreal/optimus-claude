...
- **Specific-Token Audit** — prevents hallucinated ports, paths, versions, and counts from reaching the reader. Build a `grounded-tokens` set from the detector's return format:
  ...
  - **Path check:** every `<dir>/<subdir>` or `<name>.<ext>` fragment in Installation / Source Dependencies / Environment Setup / Running in Development / Common Issues prose must either (a) be in the grounded paths set verbatim, OR (b) still exist on the filesystem — perform a `Glob` for the literal fragment at audit time. Reject paths that fail both checks (per the *Never assert an unobserved path* Content Principle).
