...
- **External services detected** (list with source AND confidence — e.g., "PostgreSQL (docker-compose.yml, confirmed), OpenIdConnect (appsettings.development.json:42, candidate)"). Confidence values come from the detector's External Services table: `confirmed` from Task 5 (compose / ORM migration sources), `candidate` from Task 5b (framework config file scan).
...
- **Environment config files found** (`.env.example`, `.env.sample`, `.env.template` for dotenv stacks; `appsettings*.json`, `application.yml`, `application.properties`, `config/*.exs`, `config/*.yml`, `config.yaml`, etc. for framework-config stacks). When listing framework config files, include the file's format (e.g., `src/Web/config/appsettings.development.json [json]`, `config/application.yml [yaml]`) so Step 4 can pick Environment Setup template (a) dotenv vs. (b) config-file-driven.
...
- **Recommended developer tools** (list the detector's Recommended Dev Tools entries — e.g., "Google Chrome, SSMS, Visual Studio" — or "none" when empty).
...
- **Humanize caveats.** Any caveat emitted from a Step-4 recipe (catalogue-fallback port/volume, moving-tag rejection, LocalStack tier gate, ARM emulation note) must describe *what the reader should know or do*, not *why the skill decided something*. Write "Port 6379 is the Redis default", not "the vendor page did not expose the port in structured form". See `external-services-docker.md` §Web-Search Recipe step 3 "Caveat rendering" for the canonical phrasings.
...
- **Explain non-idiomatic wrapper commands.** When the detected dev/build/test command is a wrapper that invokes non-obvious additional steps (e.g., `npm run serve` that expands to `npm run export-scss-vars && ng serve`, `make dev` that runs a compose-up plus a migration plus a dev server), render a single-line `> <wrapper> runs: <expanded form>` explanation below the code block. Skip this when the wrapper is a direct alias to the idiomatic tool invocation (e.g., `npm run dev` → `next dev`).
