...
5. **External Services** — how to start required infrastructure. Render per `how-to-run-sections.md` §External Services (Branch A when compose covers all services; Branch B when there is no compose file; Hybrid when compose covers only some). For per-service classification, snippet templates, the web-search recipe, citation format, registry allowlist, and sanitization rules, follow `external-services-docker.md` — it is the single source of truth for those rules. Never take image names or tags from model memory; always re-verify via the recipe. For credentials: always write literal placeholder values (`<placeholder>` or `<NAMED_PLACEHOLDER>`) and never copy or synthesize real password values into the file.
...
- **Verify External Services Docker snippets:** every Docker image reference written to `HOW-TO-RUN.md` is followed by a `- Source: [<title>](<url>)` citation per the format in `external-services-docker.md`; every image reference's registry host matches the registry allowlist in `external-services-docker.md` using the *Host Extraction and Allowlist Match* algorithm defined there (bare names with no registry are accepted as `docker.io`); no image reference uses a bare `:latest` tag when a stable versioned tag was available in the cited vendor page; each snippet's required env vars (e.g., `ACCEPT_EULA=Y`, `MSSQL_SA_PASSWORD`, `POSTGRES_PASSWORD`) are present; no citation title or inline note contains unescaped backticks, code fences, or markdown-link brackets outside the explicit fenced `docker run` block (sanitization re-check per Web-Search Recipe step 5). If the run was air-gapped and the LLM-knowledge fallback was used, the snippet is marked "inferred (not web-verified)".
