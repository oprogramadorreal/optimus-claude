...
- Do not emit a per-service `AskUserQuestion` here — the reference file documents the single batched exception for rule 4's post-web-search re-confirmation (handled in Step 4).
...
Before generating content, run `external-services-docker.md` §Web-Search Recipe for every Step 3 row whose Recommended runtime is **Docker-preferred** or whose Alternative is **Docker (offline)**. Apply §Decision Heuristics rule 4 for batched-downgrade prompts (a single `AskUserQuestion` summarising all downgraded services, before Step 5 writes the file).
...
- **Verify External Services subsections** — re-apply each rule below against every External Services subsection written to `HOW-TO-RUN.md` (Docker-snippet templates AND the `Shared-cloud, no Docker alternative` template, both of which carry a `- Source:` citation):
  - *Citation Format* and *URL host allowlist* — per `external-services-docker.md` §Citation Format. Applies to every `- Source: [<title>](<url>)` line, regardless of which template emitted it.
  - *Image reference + Host Extraction and Allowlist Match* — per `external-services-docker.md` §Web-Search Recipe step 4 and §Registry Allowlist. Applies to every `docker run` snippet's image reference. After the regex passes, also reject any path segment equal to `.` or `..`.
  - *Per-field sanitization* — re-apply `external-services-docker.md` §Web-Search Recipe step 5 regexes to every `-e`, `-p`, `-v`, image-reference, and `- Source:` line. After the volume-path regex passes, also reject any path segment equal to `.` or `..`.
  ...
  - Volume-path slug check (Step 6 only): every `-v` flag's left side must match `^[A-Za-z0-9][A-Za-z0-9._-]*-[A-Za-z0-9][A-Za-z0-9._-]*-data$` (the rendered `<project-slug>-<service-slug>-data` shape, with placeholders substituted by Step 5).
