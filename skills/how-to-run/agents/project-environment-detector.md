...
- **Node / TypeScript:** for each `package.json`, emit a row when `scripts.start` OR `scripts.dev` OR `scripts.serve` OR `bin` field is present. Prefer `dev` > `serve` > `start` as the `Start command`. (No additional skip rule is needed — when none of those scripts and no `bin` field is present, the emit condition above already fails, so a "pure library" package is excluded by construction.)
...
- **Component:** validate with `^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$` (first character alphanumeric; remaining characters letters / digits / `.` / `_` / `-`). Space is intentionally excluded — Procfile process names (Heroku grammar `[A-Za-z0-9_]+`), Cargo `[[bin]].name` (Rust crate naming), Python console-script names (PEP 8), Go `cmd/<name>` directories, and Elixir umbrella app directories never legitimately contain spaces, and accepting them here would split a `cargo run --bin <name>` / `dotnet run --project <path>` rendered command into multiple shell arguments at the space boundary. The project-name field's regex (line 277) keeps space because README-heading-derived project names can contain spaces — that field is not interpolated into shell commands. Reject otherwise.
...
- **Start command — sanitization:** every untrusted substring interpolated into the Start command (`<script>` from a package.json script key, `<name>` from a Cargo `[[bin]].name`, a Python console-script entry name, `<worker-script>` from a BullMQ npm-script wrapper) MUST match the Component allowlist defined above (currently `^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$`) — re-use the same regex so a future widening of the Component allowlist does not silently drop interpolated values that the Component check accepts. For the Procfile RHS (which legitimately contains shell syntax like `|`, `>`, `&&`), the allowlist is replaced by three explicit passes: (1) collapse any run of 3+ consecutive backticks to a single backtick; (2) strip every newline, carriage return, NUL, and other Cc/Cf control character; (3) drop the row if the resulting string contains any byte outside printable ASCII. Rows that fail sanitization are omitted — do NOT emit a row with a "sanitized" marker. Rationale: the Start command is rendered inside a `` ```bash `` fence in the main skill's output, and a leaked triple-backtick or CR byte would break fence containment and enable markdown injection in previewers.
...
- **Requires (components):** comma-separated list of other component names this component's config references. Detection: grep the component's config files (`appsettings*.json`, `environment*.ts`, `application*.yml`, `config/*.exs`, `config.*`, and the dotenv-template allowlist `.env.example` / `.env.sample` / `.env.template` ONLY) for `http://localhost:<port>` / `http://127.0.0.1:<port>` / bare `:<port>` in connection-string-looking strings, and match each `<port>` against another component's Runtime Ports row. Do NOT grep `.env`, `.env.local`, or any `.env.*.local` file — those may contain real secrets, and item 6's environment-config detection task explicitly forbids reading them; this task must respect the same boundary even though it only extracts integer ports today, so a future maintainer extending the grep cannot inherit a contradiction. Empty string `—` if none.
