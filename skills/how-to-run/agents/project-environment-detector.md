...
- `local-windows-auth` — the connection string contains `Integrated Security=True`, `Integrated Security=SSPI`, or `Trusted_Connection=Yes` (case-insensitive) AND no `User Id=` / `User=` / `Username=` clause. Windows-only auth mode that has no equivalent inside a Linux container.
- `local-socket` — the connection string references a Unix domain socket (`/var/run/postgresql/.s.PGSQL.5432`, `unix:/…`), a Windows named pipe (`\\.\pipe\<name>`), or a MongoDB Unix-socket URI (`mongodb://%2Ftmp%2Fmongo.sock`, `mongodb+unix://…`). Transport modes the standard Docker publish form cannot reproduce as-is.
