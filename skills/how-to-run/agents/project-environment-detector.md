...
- `local-windows-auth` — the connection string contains `Integrated Security=(True|Yes|SSPI)` or `Trusted_Connection=(Yes|True|1)` (case-insensitive) AND no `User Id=` / `User=` / `Username=` clause. Windows-only auth mode that has no equivalent inside a Linux container.
