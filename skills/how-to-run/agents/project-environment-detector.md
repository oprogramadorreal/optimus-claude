...
Many projects (Spring Boot, ASP.NET Core, Rails, Phoenix/Elixir, Laravel, Go-with-Viper, and any polyglot stack where services are shared-cloud rather than compose-managed) wire their external services through the application's config file, not through `docker-compose.yml` or an ORM migration tool. Scan these files so those services are not silently dropped. Run this task after Task 5 and before infrastructure detection in item 6.
   ...
   - **What to extract per match:** the section name (exactly as it appears in the file, then title-cased for the report), source `<file>:<line>`, one representative endpoint (first FQDN seen in the section's values, else the string `shared-cloud endpoint`), a best-guess type (`database` / `cache` / `queue` / `storage` / `identity` / `api` / `email` / `search` / `license` / `other`), and `Endpoint semantics` per the return-format spec — for `database`-type matches only, scan the section's connection-string-shaped values (any value where the key name matches `(?i)connection.?string|datasource|connectionuri|connection_uri|database.?url|dburl|mongodb_url|mongodb_uri|mongo_uri|server` and the value's first character isn't `{` / `[`) and apply these classifiers in order: case-insensitive presence of `Integrated Security=True` or `Trusted_Connection=Yes` AND no `User Id=` / `User=` / `Username=` clause → `local-windows-auth`; case-insensitive presence of `\\.\pipe\` or `unix:/` or `/var/run/postgresql/` or `?directConnection=true` → `local-socket`; case-insensitive match against `localhost\\[A-Za-z0-9_-]+` or `(local)\\[A-Za-z0-9_-]+` → `local-named-instance`; presence of `localhost` or `127.0.0.1` (no socket, no Windows-auth) → `local-default`; otherwise (FQDN value matches the hostname regex earlier in this task) → `remote`. Drop to `ambiguous` if the file is unreadable or no value matches any classifier. Non-database matches emit `—`. Sanitize the section name with the shared top-level-section-name allowlist `^:?[A-Za-z_][A-Za-z0-9_.-]{0,63}$` (permits an optional leading `:` for Elixir atom keys and `.` within the name for Spring `application.properties` dotted keys and PHP array dot-keys; still rejects backticks, angle brackets, whitespace, parentheses, and markdown link/image metacharacters) before title-casing — drop the match if it fails, so crafted keys cannot reach the rendered H3 heading or `(candidate)` marker. For Spring `application.properties` / `application-*.properties` files, treat the substring before the first `.` as the effective top-level section (so `spring.datasource.url` collapses to `spring`) and apply the allowlist to that collapsed form; this matches how readers think about Spring top-level namespaces.
...
Emit one row in the Components table of the return format per runnable component (web, worker, scheduler, CLI, frontend). Surfacing each component lets the main skill's *Running in Development* section pick its layout from the row count (see the *Component count → layout* table in how-to-run-sections.md §Running in Development) instead of documenting only the primary binary. The `Requires (components)` field below matches config-file port references against the Runtime Ports rows populated by Task 5c above.
...
This table drives the main skill's *Running in Development* section — see the *Component count → layout* table in how-to-run-sections.md §Running in Development. Populate via Task 5d (Runnable component enumeration). Emit rows in topological order (roots first). When zero rows exist, the main skill omits the section entirely (the repo is a library).
...
| [e.g., OpenIdConnect] | [e.g., src/Web/config/appsettings.development.json:42] | [—] | [identity] | [candidate] | [remote] |
...
`Endpoint semantics` values (database-type rows; non-database rows emit `—`):

- `docker-compose` — the row's `Source` is `docker-compose.yml` / `compose.yml`. The compose file is already Docker-aware; no connection-string shift is needed for the recommended runtime.
- `local-default` — the committed connection string in the source config file points at `localhost` / `127.0.0.1` with the image's documented default port (e.g., `localhost:5432` for Postgres, `localhost:27017` for Mongo). No Windows-auth markers, no socket / named-pipe transport.
- `local-named-instance` — SQL Server only — the committed connection string contains `localhost\<instance>` or `(local)\<instance>` (named-instance + SQL Browser); the value cannot be reproduced inside a Linux container.
- `local-windows-auth` — the connection string contains `Integrated Security=True` or `Trusted_Connection=Yes` (case-insensitive) AND no `User Id=` / `User=` / `Username=` clause. Windows-only auth mode that has no equivalent inside a Linux container.
- `local-socket` — the connection string references a Unix domain socket (`/var/run/postgresql/.s.PGSQL.5432`, `unix:/…`), a Windows named pipe (`\\.\pipe\<name>`), or a MongoDB direct-connection form (`mongodb://localhost/?directConnection=true&ssl=false`). Transport modes the standard Docker publish form cannot reproduce as-is.
- `remote` — the committed connection string's hostname is a real FQDN (matches Task 5b's hostname regex) — the project already points at a remote endpoint, no shift needed.
- `ambiguous` — the source config file was unreadable, the connection-string key was not locatable, or the value didn't match any of the classifications above. The main skill treats this as `local-default` for downstream decisions but appends a "verify-the-connection-string" caution to the Step 3 assessment.

The main skill's Pre-Conditions block (see [`../references/external-services-docker.md`](../references/external-services-docker.md) §Pre-Conditions Block) reads this field at Step 4 to decide whether a connection-string shift callout must render above the service's snippet.
...
| [e.g., fixtures/initial_data.json] | [fixtures/] | fixture-load | [python manage.py loaddata fixtures/initial_data.json] |
...
`Bootstrap mechanism` is one of:

- `raw-sql` — `*.sql` file under repo root / `db/` / `database/` / `scripts/sql/` / `data/` / `bootstrap/` / `sql/`. Driven by `sqlcmd`/`psql`/`mysql`/`mongosh` against the destination DB.
- `seed-script` — language-specific seed files (`db/seeds.rb` for Rails, `priv/repo/seeds.exs` for Phoenix/Ecto, `seed.ts`/`seed.js`/`seed.mjs` at repo root, `prisma/seed.*`, `scripts/seed.*`). Driven by the language/framework's seed runner (`rails db:seed`, `mix ecto.seed`, `tsx prisma/seed.ts`, `node scripts/seed.js`, etc.).
- `fixture-load` — declarative fixture files loaded by a framework command (`fixtures/*.json` / `fixtures/*.yaml` for Django `loaddata`).

This column drives the main skill's Schema Bootstrap pick-one rule (see [`how-to-run-sections.md`](../references/how-to-run-sections.md) §Schema Bootstrap). The detector reports the file's classification; the rendering rule combines this with the *Database migrations* row in *Dev Workflow Signals* (which captures detected ORM tools like `prisma`, `alembic`, `flyway`, `ef`) to pick exactly one mechanism as primary and demote the rest.
