...
4. **Runtime version constraints** from manifests: `engines.node`, `python_requires`, `rust-version`, `environment.sdk`, `toolchain.channel` in `rust-toolchain.toml`, `go.mod` `go` directive, `.java-version` / `pom.xml` `maven.compiler.source`, and similar. One row each; Source = `<file>:<line>` (line required — Step 6 re-reads it).
   ...
   **Version-manager files are authoritative when the manifest is silent** — emit a row cited `Source: <version-manager-file>` (no line needed):
   ...
   - `.nvmrc` / `.node-version` → `Node.js selected version: <content>`; preserve exact, major-only, or named selectors such as `20.11.1`, `20`, and `lts/iron`. A selected version is not an open-ended compatibility minimum.
   ...
   **File sanitization:** reject files larger than 1 KB. For `.tool-versions`, split each line on whitespace into a `<runtime> <version>` pair. Validate each version token against `^[A-Za-z0-9._+/-]{1,32}$` (permits aliases like `lts/iron`), or allow the exact nvm selector `lts/*`; after the regex, split on `/` and reject empty/`.`/`..` segments. Drop tokens that are empty, contain interior whitespace, or contain other shell metacharacters (the value is rendered, never executed). Never parse `#` comment lines as versions.
...
7. **Aggregation:** monorepos — aggregate services and dependencies across subprojects; multi-repo workspaces — per-repo context plus a whole-workspace synthesis (all repos' services, shared infrastructure, cross-repo dependencies).
   ...
   **Go workspaces:** read the `use` entries in `go.work` and report each actual module directory as a Subprojects row (Path = the module directory, Package manager = `go (go.work)`). Resolve paths relative to `go.work`, preserve spaces, and confirm the directory and its `go.mod` exist within the authorized workspace. Mark paths outside the readable scope as unverified. Do not assume the workspace root itself is a module; commands must address the listed modules.
