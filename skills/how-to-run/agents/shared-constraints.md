...
The base constraints — read-only analysis (modify nothing, create nothing, run nothing that changes state) and "only report what you are confident about; your results are independently validated against the project" — are the **Agent Constraints** in `references/shared-agent-constraints.md` at the plugin root; the main skill prepends that section at dispatch.
