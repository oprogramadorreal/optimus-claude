...
- Also discovers external services from framework config files (`appsettings*.json`, `application.yml`, `config/*.exs`, Rails `config/*.yml`, Laravel `config/*.php`) — rendered with a `(candidate)` marker so compose-confirmed services and framework-config candidates are distinguishable
- When no `docker-compose.yml` covers a service, classifies each one as **Docker-preferred**, **Shared-cloud primary** (with Docker as offline alternative when a vendor image exists, otherwise shared-cloud only), or **Local install only**, and emits a `docker run` snippet with a vendor-cited image reference (never guessed from model memory). Vendor-branded cloud services (AWS S3/SNS/SQS, Azure Cosmos DB, Firebase, GCP Pub/Sub) resolve to their local emulators (LocalStack, Azurite, Firebase Local Emulator Suite, etc.) via the Vendor-Service → Emulator Index. GUI clients and CLI tools (see [`references/external-services-docker.md`](references/external-services-docker.md) §Service Classification Tables) always render as local install. Registry allowlist bounds supply-chain risk.
...
- Detects recommended developer tools (browsers, IDEs, DB GUI clients, API debuggers) mentioned in existing READMEs and surfaces them under Prerequisites
- Enumerates all runnable components, not just the primary binary — ASP.NET web + background workers, Rails web + Sidekiq, Go `cmd/*/main.go`, Cargo `[[bin]]` entries, Procfile lines — and renders *Running in Development* with a layout that scales from a flat block for 1-2 components to a Boot-order list for 3-5 and a quick-reference table for 6+.
- Detects bound runtime ports from launch configs (.NET `launchSettings.json`, Rails `puma.rb`, Spring Boot `server.port`, Node `app.listen(…)`, Go `http.ListenAndServe`, etc.) so `Expected result:` URLs show the real port — never a framework default guessed from model memory.
- Captures workspace kind (`npm-workspaces` / `pnpm-workspaces` / `cargo-workspace` / `go-workspace` / `gradle-multi-module` / `maven-multi-module` / …) and renders the matching install / build / run commands (`cargo build --workspace`, `go work sync`, `./gradlew build`, `npx nx run-many -t build`) instead of falling back to the per-package PM.
- Treats version-manager files as authoritative — `.python-version`, `.ruby-version`, `.nvmrc`, `rust-toolchain.toml`, `.tool-versions` (asdf/mise) — when the manifest has no `engines` / `python_requires` equivalent, so Prerequisites cites a real pinned version instead of "recent LTS".
- Detects setup rituals — one-shot bootstrap scripts (`bootstrap.sh`, `bin/setup`, `scripts/bootstrap.*`), Dev Containers (`.devcontainer/devcontainer.json` — promoted to a *Quick start* block above the manual flow), pre-commit hooks, direnv (`.envrc`), and mkcert (local HTTPS bootstrap) — and surfaces each as a Prerequisites or Installation note.
- Step 6 Specific-Token Audit: every port, path, version, and exact count in the generated doc must either be grounded in a detector return-format citation (with `<file>:<line>` re-read at audit time) or be re-observable via `Glob`. An Unverified-Count regex filter rejects prose like "15 `.csproj` projects" or "47 modules" unless the number matches a live enumeration. Eliminates the most common class of hallucinated token (framework-default ports, non-existent version folders, unverified module counts).
...
When `HOW-TO-RUN.md` already exists, the skill offers three choices: a guided in-chat walkthrough of the documented steps (display-only — the user runs each command locally; the skill never executes anything), regenerate the doc, or skip. The walkthrough surfaces the audit verdict per step and flags destructive or remote-fetch commands as advisories — see [`references/guided-walkthrough.md`](references/guided-walkthrough.md). It never modifies `HOW-TO-RUN.md` or any other file in the project.
...
1. **Prerequisites** — OS version constraints, hardware requirements, system tools, version managers, plus a *Recommended developer tools* sub-list when browsers, IDEs, DB GUI clients, or API tools are mentioned in existing docs
...
5. **External Services** — docker-compose services with ports and purpose, or per-service Docker-preferred / Shared-cloud primary / Local install only rendering when compose does not cover the service (with vendor-cited image references). When the committed connection string is incompatible with the chosen runtime (Windows authentication, named instance, Unix socket), emits a Pre-Conditions Block describing the required override before any Setup command runs. Appends a one-line PowerShell quoting caveat on Windows projects, and a one-line "From <tool>" connect note when a detected DB GUI client (SSMS, pgAdmin, DBeaver, etc.) matches a service. Framework-config candidates render with a `(candidate)` marker.
...
10. **Common Issues** — version manager hints, service startup reminders, private registry auth, submodule reminders, `Verify <service>` bullets for Docker-preferred services, and diagnostic ladders for multi-layer failure modes (e.g., "container running but host can't connect")
...
| `references/step6-verification-audits.md` | Step 6 audit rule bodies: External Services re-verification, section ordering, Pre-Conditions Block, detector-token re-validation, Specific-Token Audit, Unverified-Count filter, template-shape audit |
