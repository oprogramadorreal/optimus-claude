...
- Enumerates all runnable components, not just the primary binary — ASP.NET web + background workers, Rails web + Sidekiq, Go `cmd/*/main.go`, Cargo `[[bin]]` entries, Procfile lines. *Running in Development* picks its layout from the component count: 1-2 components render as flat sub-templates with no shells; 3-5 components render as a *Compact multi-component layout* — a numbered Boot-order list with one bullet per component (start command, expected URL/port, requires); 6+ components fall back to a quick-reference table (§Scaling Guidance). The legacy H4-per-component renderer is gone — it inflated the section to ~12 lines per component without adding signal.
