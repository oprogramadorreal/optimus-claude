...
5. **External Services** — docker-compose services with ports and purpose, or per-service Docker-preferred / Shared-cloud primary / Local install only rendering when compose does not cover the service (with vendor-cited image references). When the committed connection string is incompatible with the chosen runtime (Windows-auth, named instance, Unix socket, port mismatch), emits a Pre-Conditions block with a generated before/after diff. Renders per-shell `docker run` snippets (bash + PowerShell) for Windows projects. Adds `Connecting from <tool>` guides for detected DB GUI clients (SSMS, pgAdmin, DBeaver, etc.). Framework-config candidates render with a `(candidate)` marker.
...
| `references/gui-client-service-catalog.md` | GUI client × service cross-product catalogue, rendering rule, and Web-Search re-validation for the `Connecting from <tool>` heading emitted inside External Services per-service subsections |
