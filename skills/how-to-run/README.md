...
- Enumerates all runnable components, not just the primary binary — ASP.NET web + background workers, Rails web + Sidekiq, Go `cmd/*/main.go`, Cargo `[[bin]]` entries, Procfile lines — and renders *Running in Development* with a layout that scales to the component count (flat for 1-2, *Compact multi-component layout* with a numbered Boot-order list for 3-5, quick-reference table for 6+).
