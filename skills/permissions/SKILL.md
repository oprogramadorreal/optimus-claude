...
Security model in brief: the installed hook prompts on writes and blocks deletes outside the project (Claude's memory store and session scratchpad are exempt — creating a new file in an invented temp dir is denied with the scratchpad path as model-facing feedback so Claude retries there, while existing temp files and `~/.claude` still prompt), asks before editing and blocks deleting precious unversioned files, and blocks history-modifying git operations on protected branches. Inside the project, operations not on the deny list run without prompts.
