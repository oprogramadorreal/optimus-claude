...
Security model in brief: the installed hook prompts on writes and blocks deletes outside the project (Claude's memory store and session scratchpad are exempt — creating a new file in an invented temp dir still prompts, with a reason reminding Claude the scratchpad is exempt), asks before editing and blocks deleting precious unversioned files, and blocks history-modifying git operations on protected branches. Inside the project, operations not on the deny list run without prompts.
