...
Two out-of-project locations are exempt (writes and deletes allowed without prompts): Claude's per-project memory store (`~/.claude/projects/.../memory/`) and the session scratchpad (`<temp>/claude/<project>/<session>/scratchpad/`). Both are Claude's own recoverable scratch space; the path matches are tightly scoped, `..`-traversal out of them is rejected, and the rest of `~/.claude` (notably `settings.json`) still prompts. Creating a **new** file under the temp root in a dir that misses the scratchpad shape — an invented temp dir — is denied with the exact exempt path as model-facing feedback instead of a user prompt, so the retry lands in the scratchpad and proceeds unprompted. That redirect is scoped so it can never swallow a decision that is yours to make: a file that already exists is never redirected (a deny goes to Claude, not to you, so you would have no way to approve an edit of a file that cannot move), `~/.claude` is never redirected even when `HOME` itself sits under the temp root (CI images), and anything that cannot be resolved to an absolute scratchpad path falls back to the normal prompt. Details in the header of [`templates/hooks/restrict-paths.sh`](templates/hooks/restrict-paths.sh).
