...
- If the prompt was for Claude Code plan mode → tell the user to paste the prompt as the **first message in a new Claude Code conversation started in plan mode**. Do not suggest pasting it in the current conversation. Treat plan mode as review-only: iterate on the plan, then **switch out of plan mode without approving** (plan-mode approval executes immediately and skips any follow-up skill like `/optimus:tdd`). For the exact client-agnostic wording on entering/exiting plan mode and the full handoff template, see `$CLAUDE_PLUGIN_ROOT/references/skill-handoff.md`.
