...
- **Seedance 2**: prose, not labeled fields (~60-100 words, Subject → Action → Environment → Camera → Lighting → Style → Audio). ONE camera move; pacing words, not specs. DROP the Negative Prompts field — unsupported; fold exclusions into positive phrasing ("clean motion, correct hands"). Native synced audio; `@Image1`/`@Video1` reference tags. For image-to-video, fill only the motion/change plus "preserve composition and colors"
