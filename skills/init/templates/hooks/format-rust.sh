...
while :; do
  ...
  parent="$(dirname "$dir")"
  [[ "$parent" == "$dir" ]] && break  # a UNC root: dirname // stays //
  dir="$parent"
done
...
if [[ -z "$explicit_edition" ]]; then
  echo "[format-rust] no explicit edition in the nearest project rustfmt config — skipped. Use the project's cargo fmt command, or set edition (and, if wanted, style_edition) there to match its toolchain." >&2
  exit 0
fi
