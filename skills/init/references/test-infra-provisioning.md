...
- **Test assertion failures** (tests compile and run, but some fail) — report but continue. These are logic bugs, not infrastructure issues. Record `{ subproject, failing_count }` per subproject and carry it into Step 7's summary (consumed by the broken-baseline modifier).
