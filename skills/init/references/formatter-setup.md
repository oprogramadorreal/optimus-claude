...
Instructions for installing auto-format hooks per tech stack.
...
**Unsupported stacks:** Apply the fallback procedure from `$CLAUDE_PLUGIN_ROOT/skills/init/references/unsupported-stack-fallback.md` to find the standard formatter via web search. If none is found, inform the user: **"Could not determine a formatter for [detected stack] — skipping formatter hook."** Do not create a hook. If the user approves the proposed formatter, create a custom shell hook (`.claude/hooks/format-<language>.sh`) following the pattern of `format-go.sh`: shebang, its JSON stdin parsing into `$file_path` copied verbatim, file-extension guard for the language's source extensions, then formatter invocation with `"$file_path"`.
...
2. External formatters not in deps → ask the user "Add [formatter] as dev dependency and install format hook?" If declined, skip that stack's hook entirely. If approved, install with the detected package manager's standard dev-dependency command. Non-obvious flows:
   - **C#/.NET (csharpier):** install as a local tool so all developers get it via `dotnet tool restore` — if `.config/dotnet-tools.json` is missing run `dotnet new tool-manifest`, then `dotnet tool install csharpier`, then `dotnet tool restore`.
     Preserve existing pinned versions.
...
4. If hooks were installed, merge one matcher group per installed hook into `.claude/settings.json`, copied from `$CLAUDE_PLUGIN_ROOT/skills/init/templates/settings.json` (a custom hook follows its `bash` entries).
