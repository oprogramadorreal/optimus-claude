...
- **Git init:** run `git init` only when Git confirms the scaffold is outside any working tree and is not a bare repository. Do not reinitialize a linked worktree or treat a Git access error as a missing repository.
