...
Launch 1 `general-purpose` Agent tool call using the agent prompt from `codebase-detection.md`, prepended with the reference file contents above.
...
Launch 1 `general-purpose` Agent tool call using the agent prompt from `documentation-audit.md`. **Provide the Detection Results from Step 1 as context** at the start of the agent prompt (before the agent template content).
