...
init is the foundation: `/optimus:tdd`, `/optimus:unit-test`, and `/optimus:brainstorm` require an initialized project (CLAUDE.md); `/optimus:refactor`, `/optimus:code-review`, and `/optimus:deep` use its guidelines and test command (falling back to general best practices when missing); `/optimus:permissions` shares `.claude/settings.json`. After init, `/optimus:how-to-run` generates the human-facing onboarding doc.
