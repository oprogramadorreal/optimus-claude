...
- **The hook command in `hooks/hooks.json` launches Bash through a Git shell alias.** Native Codex can use PowerShell, not just `cmd.exe`: `||` fails in Windows PowerShell 5.1, and nested backslash-escaped quotes are not portable to PowerShell 7. The alias body is only `!exec bash`; the script path is a separate argument, so Git handles its shell quoting. Git for Windows supplies Bash without selecting a broken WSL `bash` on PATH. Claude Code's schema rejects unknown keys, ruling out `commandWindows` in the shared config. Git starts shell aliases at the repository root: `--git-alias` tells the hook to restore the caller's directory using `GIT_PREFIX` before inspecting project state. Direct script launches ignore that variable.
