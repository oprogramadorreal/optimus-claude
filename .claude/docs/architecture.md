...
- Each iteration (deep variant) or cycle (paired variant):
  ...
  4. The orchestrator saves the subagent's output to a temp file, runs `cli parse` to extract the JSON, then `cli deep-step` (or `refactor-step` for the paired refactor phase) to apply fixes, run tests, bisect on failure, and update statuses; `unit-test-step` instead records tests + coverage, rolling the whole cycle back if the suite is red. Whenever a repeatable git snapshot exists (commit mode), the bisect rebuilds each candidate state from that snapshot rather than trusting the reported `pre/post_edit_content` as revert data — a corrupt record can only fail loudly as `skipped`, never tear the tree.
