...
if [ -n "$py_cmd" ]; then
  ...
  done < <({ find ./skills -path '*/templates/*.py' -o -path '*/templates/**/*.py' 2>/dev/null; find ./scripts -name '*.py' 2>/dev/null; find ./scripts/test-coverage-harness -name '*.py' 2>/dev/null; } | sort -u)
else
