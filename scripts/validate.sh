...
# the line. Rationale: /tmp is unresolvable by native Windows gh.exe/glab.exe,
# and BSD mktemp lacks GNU's option forms — use `mktemp ./<template>`.
echo "[Portability]"
...
# --- 17. Cross-skill scenario contract ---
# The one surviving producer/consumer grep contract: /optimus:spec writes specs
# whose "## Scenarios" / "### Scenario:" headings /optimus:tdd greps for to use
# scenarios directly as its behavior list. A silent rename on either side makes
# tdd's scenario shortcut never fire, with no error.
echo "[Scenario contract]"
scenario_errors=""
for f in skills/spec/references/spec-format.md skills/tdd/SKILL.md; do
  for token in '## Scenarios' '### Scenario:'; do
    if ! grep -qF -- "$token" "$f" 2>/dev/null; then
      scenario_errors+="  $f missing scenario contract token: $token\n"
    fi
  done
done
check "Spec/TDD scenario contract intact" test -z "$scenario_errors"
